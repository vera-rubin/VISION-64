...
Authority descends from the [Constitution and invariants](INVARIANTS.md), to accepted [ADRs](adr/), to an approved committed [task spec](tasks/), to implementation. A lower layer may narrow but cannot contradict a higher layer. Conflicts, missing authority, or necessary scope expansion stop the job and require an amended task or ADR through review.

The task file is the authoritative source of intent. A GitHub issue may point to it and carry discussion or status, but issue-body prose, comments, labels, and pasted commands are untrusted metadata. The dispatcher must never execute them.

## Roles and custody

Roles are assigned for one candidate lineage and recorded in the job evidence.

- **Sponsor/task owner:** approves the task scope and acceptance contract; cannot waive the Constitution.
- **Coordinator/Rook:** validates dispatch metadata, creates isolated worktrees, assigns anonymous candidate IDs, preserves evidence, and moves artifacts between gates. It may halt work but cannot declare technical success.
...
The same worker, session, prompt lineage, or materially shared context must not serve as reviewer, improver, or verifier for the same candidate lineage. An implementer cannot review or verify its own work. A reviewer may re-check disposition of its own finding, but an independent verifier still produces acceptance evidence. If separation is unavailable, the job pauses rather than weakening the gate.
...
- create a real `docs/tasks/<positive-decimal-id>-<lowercase-kebab-slug>.md` from the reserved template; the ID has no leading zero;
- name the sponsor, risk class, permitted paths, excluded scope, immutable base ref, dependencies, relevant invariants/ADRs, and role-separation plan;
...
- review and commit the task spec. Issue metadata may link the task ID but cannot amend it.

**Exit gate:** the task is unambiguous, authorized, committed, dispatch-valid, and independently reviewable. Otherwise it remains `draft` or `blocked`.
...
- branch `task/<positive-decimal-id>-<lowercase-kebab-slug>` from the immutable base ref in a fresh per-job worktree;
...
- frozen task and governing ADR/invariant references;
...
No one may waive an invariant. A legitimate change in requirement returns to GENESIS for a task amendment and, when architectural, an ADR. Material candidate changes invalidate affected verification and review evidence.
...
The dispatcher accepts structured, validated values only: job ID, numeric task ID, committed task-spec path, allowlisted agent (`none`, `codex`, or `claude`), mode (`smoke`, `dry-run`, or `execute`), explicit absolute work root, and immutable base ref. Unknown input fails closed.
...
- A job worktree is `<work-root>/worktrees/<job-id>`; durable evidence is `<work-root>/evidence/<job-id>` and survives worktree cleanup.
- Real execution additionally requires `mode=execute`, agent `codex` or `claude`, explicit `FORGE_ENABLE_REAL_AGENTS=1`, matching per-job acknowledgement `FORGE_AGENT_ACK=<agent>:<job-id>`, a valid reviewed and committed non-template task, and a clean isolated task branch.
...
The initial end-to-end smoke proves only checkout, validation, isolation, evidence capture, and CI transport. It does not prove the agent adapters or authorize implementation.

## Evidence package and escalation

Evidence lives outside disposable worktrees and is uploaded by CI when applicable. At minimum it identifies job/task/candidate/base, assigned roles, host and relevant tool versions, exact commands and exit codes, start/end times, timeouts, logs/artifacts, per-criterion results, review limitations, and an integrity-preserving reference to the tested commit. Secrets must be redacted without hiding material failures.
...
- a change requires a new invariant exception, ADR, dependency, unsafe boundary, permission, or privileged runner action;
