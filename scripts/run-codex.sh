...
if [[ "$mode" == execute ]]; then
    ...
    [[ "${FORGE_AGENT_ACK:-}" == "codex:$job_id" ]] || die "set FORGE_AGENT_ACK=codex:$job_id for this job"
fi

allow_file="$evidence_dir/codex-allowed-paths.txt"
...
} >"$evidence_dir/codex-request.env"

prompt_file="$evidence_dir/codex-prompt.txt"
{
    printf '%s\n' 'You are an implementation worker in the VISION-64 FORGE pipeline.'
    printf '%s\n' 'Read and obey AGENTS.md before editing; repository rules remain binding.'
    ...
    printf '%s\n' 'Stop and report a blocker if the task requires anything outside those boundaries.'
    printf '\nAllowed paths:\n'
    sed 's/^/- /' "$allow_file"
...
if [[ "$mode" == dry-run ]]; then
    printf 'codex dry-run validated; no agent invoked\n'
    exit 0
fi
...
codex --version >"$evidence_dir/codex-version.txt" 2>&1 || die 'unable to record codex version'
codex_help=$(codex exec --help 2>&1) || die 'unable to inspect codex exec capabilities'
for required_flag in '--sandbox' '--ephemeral' '--json' '--ignore-user-config'; do
    grep -F -- "$required_flag" <<<"$codex_help" >/dev/null ||
        die "installed codex is missing required flag: $required_flag"
done
...
(
    ...
    env -i HOME="${HOME:?}" PATH="${PATH:?}" LANG=C.UTF-8 LC_ALL=C TERM=dumb \
        ...
        codex exec --ignore-user-config --sandbox workspace-write --ephemeral --json - <"$prompt_file"
) >"$evidence_dir/codex-events.jsonl" 2>"$evidence_dir/codex-stderr.txt"
agent_status=$?
set -e
printf 'exit_code=%s\n' "$agent_status" >"$evidence_dir/codex-result.env"
((agent_status == 0)) || die "codex exited with status $agent_status"

printf 'codex execution completed\n'
